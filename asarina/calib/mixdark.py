#!/usr/bin/env python3
"""
Combine dark frames with IRAF, handling arbitrary numbers of frames.
...
2. Combines each chunk in parallel using iraf_combine_dark.sh
...
Usage:
    mixdark.py output.fits input1.fits input2.fits ...
    mixdark.py output.fits *.fits
"""
...
IRAF_SCRIPT = SCRIPT_DIR / "iraf_combine_dark.sh"
...
def combine_final(partial_files: list[str], output_path: str) -> bool:
    ...
    Since each partial is already CR-cleaned, we can use simple averaging
    or the same IRAF approach (which will work fine for 2-3 images).
    """
    ...
    # With only 2-3 images, the minmax rejection will be minimal but that's fine
...
def main():
    parser = argparse.ArgumentParser(
        description="Combine dark frames with IRAF, handling arbitrary numbers of frames."
    )
    ...
    tempdir = tempfile.mkdtemp(prefix="mixdark_", dir=user_tmp)
    try:
        partial_outputs = []

        # Run chunks in parallel (limit to 4 by default - IRAF is I/O bound)
        n_jobs = args.jobs or min(n_chunks, MAX_WORKERS)
        if args.verbose:
